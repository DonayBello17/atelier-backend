...
router.post('/:id/activar', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Error al activar cliente',
    });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (Number(compras[0].total) > 0) {
      return res.status(400).json({
        success: false,
        message: 'No puedes eliminar un cliente con ventas registradas. Puedes desactivarlo para conservar el historial y las facturas.',
      });
    }
